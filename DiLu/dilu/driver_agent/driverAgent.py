...
class DriverAgent:
    def __init__(
        self, sce: EnvScenario,Amy_Steve,other_llm,dj_llm,
        temperature: float = 0, verbose: bool = False
    ) -> None:
        self.sce = sce
        self.other_llm = other_llm
        self.Amy_Steve = Amy_Steve
        self.dj_llm = dj_llm
    ...
    def few_shot_decision(self,scenario_description: str = "Not available", previous_decisions: str = "Not available", available_actions: str = "Not available", driving_intensions: str = "Not available", fewshot_messages: List[str] = None, fewshot_answers: List[str] = None):
        ...
        You'll receive a thorough description of the current driving scenario, including your past decisions and insights into the intentions of others in this situation. You will also be given the available actions you are allowed to take. All of these elements are delimited by {delimiter}.

        Your response should use the following format:
        <considering Steve and Amy's opinions>
        <considering other llms' opinions>
        <reasoning>
        <reasoning>
        ...
        Here is the opinions of Steve and Amy and other llms:
        {delimiter} Steve and Amy's opinions:
        {self.Amy_Steve}
        {delimiter} jury's opinions:
        {self.other_llm}
        {delimiter} deputy judge's opinions:
        {self.dj_llm}
        ...
        print("\n")
        response = response_content.split(delimiter)[-1]
        print("response:", response)
        # if "Amy" in response:
        #     decision_person = "Amy"
        # elif "Steve" in response:
        #     decision_person = "Steve"
        # else:
        #     decision_person = "None"
        decision_action = re.findall(r'\d+', response)
        print("decision_action:", decision_action)
        try:
            result = int(decision_action[0])
