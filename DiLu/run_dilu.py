...
def whole(env, other_llm, Amy_Steve, dj_llm):
    import warnings
    warnings.filterwarnings("ignore")

    config = yaml.load(open('DiLu/config.yaml'), Loader=yaml.FullLoader)
    ...
    envType = 'highway-fast-v0'
    seed = random.choice(test_list_seed)
    sce = EnvScenario(env, envType, seed, database_path)
    DA = DriverAgent(sce, Amy_Steve, other_llm, dj_llm, verbose=True)
    if REFLECTION:
...
if __name__ == '__main__':
    ...
    while episode < config["episodes_num"]:
        ...
        seed = random.choice(test_list_seed)
        obs, info = env.reset(seed=seed)#改为传入参数
        env.render()
